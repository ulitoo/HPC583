...
rows, cols = 14, 7   # Adjust these dimensions based on your data
...
x_values2 = np.array(double_matrix_lapack)[0:13, 0]


# Extract the Column from each matrix
Residual_mine = np.array(double_matrix_mine)[0:13,1]
...
Time_mine = np.array(double_matrix_mine)[0:13,6]
...
axs[0, 0].plot(x_values2,Residual_mine, label='Residual Norm Mine')
axs[0, 0].plot(x_values,Residual_lapack, label='Residual Norm LAPACK')

# Add labels and legend
axs[0, 0].set_title('Residual Norm for my implementation vs LAPACK')
...
axs[0, 1].plot(x_values2,Fwd_Error_mine, label='Fwd Error Mine')
axs[0, 1].plot(x_values,Fwd_Error_lapack, label='Fwd Error LAPACK')

# Add labels and legend
axs[0, 1].set_title('FWD Error for my implementation vs LAPACK')
...
axs[1, 0].plot(x_values2,Time_mine, label='Time Mine')
axs[1, 0].plot(x_values,Time_lapack, label='Time LAPACK')

# Add labels and legend
axs[1, 0].set_title('Time for my implementation vs LAPACK')
