...
/////////////////////////////// SCALAPACK Distributions

void ScatterMatrix(int context, float *globalMatrix, int M, int N, int Mb, int Nb, float *localMatrix, int localrows, int localcols, int myprow, int mypcol, int nprow, int npcol)
{
    ...
    for (int r = 0; r < M; r += Mb, sendr = (sendr + 1) % nprow) // go jumping global matrix block by block (row)
    {
        ...
        for (int c = 0; c < N; c += Nb, sendc = (sendc + 1) % npcol) // go jumping global matrix block by block (col)
        {
            ...
            {
                // void Cdgesd2d(int context, int M, int N, const float* A, int lda, int rdest, int cdest);
                Csgesd2d(context, nr, nc, &globalMatrix[COL_MAJOR_INDEX(r, c, M)], M, sendr, sendc); // sendr,sendc are processes to send to
            }

            if (myprow == sendr && mypcol == sendc) // if caller to scatter is the corresponding process  (sendr,sendc) then receive
            {
                // void Cdgerv2d(int context, int M, int N, float* A, int lda, int rsrc, int csrc);
                Csgerv2d(context, nr, nc, &localMatrix[COL_MAJOR_INDEX(recvr, recvc, localrows)], localrows, 0, 0); // receive from root 0,0
...
void CollectMatrix(int context, float *globalMatrix, int M, int N, int Mb, int Nb, float *localMatrix, int localrows, int localcols, int myprow, int mypcol, int nprow, int npcol)
{
    int sendr = 0;
    int sendc = 0;
    int recvr = 0;
    ...
    for (int r = 0; r < M; r += Mb, sendr = (sendr + 1) % nprow)
    {
        ...
        for (int c = 0; c < N; c += Nb, sendc = (sendc + 1) % npcol)
        {
            ...
            {
                Csgesd2d(context, nr, nc, &localMatrix[COL_MAJOR_INDEX(recvr, recvc, localrows)], localrows, 0, 0);
                recvc = (recvc + nc)% localcols;
            }

            if (myprow == 0 && mypcol == 0)
            {
                Csgerv2d(context, nr, nc, &globalMatrix[COL_MAJOR_INDEX(r, c, M)], M, sendr, sendc);
            }
        }
...
float Fwd_Error_diff(float *matrixA, float *matrixB, int m, int n)
{
    float epsilon = float_machine_epsilon();
    float *matrixResidual = (float *)malloc(m * n * sizeof(float));
    float residual_norm,A_norm,B_norm;
    for (int i = 0; i < m*n; i++)
    {
    ...
    A_norm = InfinityNorm(matrixA,m);
    B_norm = InfinityNorm(matrixB,m);

    free(matrixResidual);

    return residual_norm/(A_norm*B_norm*epsilon);
}
float InfinityNorm_Error_diff(float *matrixA, float *matrixB, int m, int n)
{
    float epsilon = float_machine_epsilon();
    float *matrixResidual = (float *)malloc(m * n * sizeof(float));
    float residual_norm,A_norm,B_norm;
