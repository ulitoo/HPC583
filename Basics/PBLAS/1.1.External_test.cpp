...
int main(int argc, char **argv){
  ...
  if (mpiroot) {
    ...
    stringstream stream;
    stream << argv[2] << " " << argv[3] << " " << argv[4] << " " << argv[5] << " " << argv[6] << " " << argv[7];
...
//  int JZ = 1;
  ...
  Z = new double[N*M];
  lwork = -1;
  pdsyev_("V", "U", &N, A_loc, &iONE, &iONE, descA, w, Z, &iONE, &iONE, descZ, &wkopt, &lwork, &info);

  lwork = (int)wkopt;
  work = new double[lwork];
  pdsyev_("V", "U", &N, A_loc, &iONE, &iONE, descA, w, Z, &iONE, &iONE, descZ, work, &lwork, &info);
  ...
  if(mpiroot){
    std::cout<<"PDSYEVD MPI Run Time" << MPIelapsed<<std::endl;
