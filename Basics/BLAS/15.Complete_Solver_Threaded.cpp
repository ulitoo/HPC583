...
void LUdecompositionDoolittle(double *A, double *L, double *U, int n) 
{
    for (int i = 0;i < n; i++)
    {
        for (int j = i;  j< n; j++)
        ...
        for (int j = i; j < n  ; j++)
        {
            ...
            else
            {
                ...
                L[j + i * n] = (A[j + i * n] - sum) / U[i + i * n];
            }
        }
    }
}

void LUdecompositionDoolittleThreaded(double *A, double *L, double *U, int n,int maxthreads)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = i; j < n; j++)
        {
            ...
            {
                threads.emplace_back([i, j, A, L, U, n]()
                                     {
                double sum = 0.0;
                for (int p = 0; p < i; p++)
                {
    ...
    int main(int argc, char *argv[])
    {
        ...
        if (argc != 5)
        {
            std::cerr << "Usage: " << argv[0] << " <filename A> <filename B> rank maxthreads" << std::endl;
            return 1;
        }
        const int n = std::atoi(argv[3]); // rank
        const int maxthreads = std::atoi(argv[4]); // rank
        // Timers
        ...
        // LUdecompositionRecursive3(matrixA, matrixL, matrixU, n);
        LUdecompositionDoolittleThreaded(matrixA, matrixL, matrixU, n, maxthreads);
        // LUdecompositionDoolittle(matrixA, matrixL, matrixU, n);
