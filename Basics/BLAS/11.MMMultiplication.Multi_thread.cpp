...
// Define a structure to pass parameters to the threaded function
struct ThreadData {
    double *matrixa;
    int matrixa_m, a_m1, a_n1;
    double *matrixb;
    int matrixb_n, b_n1, b_p1;
    double *matrixc;
    int matrixc_m, c_m1, c_p1;
    int m, n, p;
    int recursion_limit;
};
...
void MMMultRecursiveT(double *matrixa, double *matrixb, double *matrixc, int m, int n, int p, int recursion_limit)
{
    recursion_count++;
    // if any of those is n=1 STOP??  Where to break recurse????
    // if n intermediate is small enough? if n==1? Break recurse??
    ...
    else
    {
        ...
        // C11 is recurse1 and recurse2
        std::thread threadC11_1(MMMultRecursiveT,A11, B11, C11, mm, nn, pp,recursion_limit);
        std::thread threadC12_1(MMMultRecursiveT,A11, B12, C12, mm,nn,pp2,recursion_limit);
        std::thread threadC21_1(MMMultRecursiveT,A21, B11, C21, mm2,nn,pp,recursion_limit);
        std::thread threadC22_1(MMMultRecursiveT,A21, B12, C22, mm2,nn,pp2,recursion_limit);
        ...
        std::thread threadC11_2(MMMultRecursiveT,A12, B21, C11, mm, nn2, pp,recursion_limit);
        std::thread threadC12_2(MMMultRecursiveT,A12, B22, C12, mm,nn2,pp2,recursion_limit);
        std::thread threadC21_2(MMMultRecursiveT,A22, B21, C21, mm2,nn2,pp,recursion_limit);
        std::thread threadC22_2(MMMultRecursiveT,A22, B22, C22, mm2,nn2,pp2,recursion_limit);
...
int main ( int argc, char* argv[] ) {
    ...
    MMMultRecursiveT(matrixA,matrixB,matrixC,m,n,p,recursion_limit);
    ...
    //MMMultRecursive3ThreadedW(matrixA,m,0,0 ,matrixB,n,0,0, matrixC,m,0,0,m,n,p,recursion_limit);    
    ...
    std::cout << "\nABS Error Threaded Recursive3 (C - Ctest)-----------------> : " << diff << "\n";
    ...
    diff=MatrixAbsDiff(matrixC_test,matrixC,m,p);
