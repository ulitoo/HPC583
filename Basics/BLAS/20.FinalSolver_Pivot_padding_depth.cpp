...
void UpperTriangularSolverRecursiveReal_3(double *matrixU, double *matrixB, double *matrixX, int n, int p)
{
    // This is a Naive version with Malloc and free as crutch to avoid index calculation over the original matrix
    // it supports odd
    // and will calculate always with even
    // assume square matrix and even in calculations
    if (n == 1)
    {
            matrixX[0] = matrixB[0] / matrixU[0];
    }
    else if (n%2==1) //odd
    {
        ...
        UpperTriangularSolverRecursiveReal_3(matrixU2, matrixB2, matrixX2, n+1, n+1);
    ...
    else
    {
        ...
        double *U11 = (double *)malloc(nn2 * sizeof(double));
        double *U12 = (double *)malloc(nn2 * sizeof(double));
        double *U21 = (double *)malloc(nn2 * sizeof(double));
        double *U22 = (double *)malloc(nn2 * sizeof(double));
        double *B11 = (double *)malloc(nn2 * sizeof(double));
        double *B12 = (double *)malloc(nn2 * sizeof(double));
        double *B21 = (double *)malloc(nn2 * sizeof(double));
        double *B22 = (double *)malloc(nn2 * sizeof(double));
        double *X11 = (double *)malloc(nn2 * sizeof(double));
        double *X12 = (double *)malloc(nn2 * sizeof(double));
        double *X21 = (double *)malloc(nn2 * sizeof(double));
        double *X22 = (double *)malloc(nn2 * sizeof(double));

        // Initializa Axx and Bxx matrices!
        InitializeSubmatrices(matrixU, U11, U12, U21, U22, n, n);
        InitializeSubmatrices(matrixB, B11, B12, B21, B22, n, n);

        // Recurse U22 X21 = B21
        UpperTriangularSolverRecursiveReal_3(U22, B21, X21, nn, nn);
        // Recurse U22 X22 = B22
        UpperTriangularSolverRecursiveReal_3(U22, B22, X22, nn, nn);
        ...
        for (int i = 0; i < nn; ++i)
        {
            for (int j = 0; j < nn; ++j)
            {
                for (int k = 0; k < nn; ++k)
                {
                    B11[i + ((j)*nn)] -= (U12[i + (k)*nn]) * (X21[k + (j)*nn]);
                    B12[i + ((j)*nn)] -= (U12[i + (k)*nn]) * (X22[k + (j)*nn]);
                }
            }
        }
        ...
        // PHASE 3: RECURSE on REST of calculations with TRIANGULAR A22
        // Recurse U11 X11 = B11'
        UpperTriangularSolverRecursiveReal_3(U11, B11, X11, nn, nn);
        // Recurse U11 X12 = B12'
        UpperTriangularSolverRecursiveReal_3(U11, B12, X12, nn, nn);

        // At the end Collect pieces of matrixc = matrixc + C11 + C12 + C21 + C22 and done!
        CollectSubmatrices(matrixX, X11, X12, X21, X22, n, n);
...
void LowerTriangularSolverRecursiveReal_3(double *matrixL, double *matrixB, double *matrixX, int n, int p)
{
    ...
    else if (n%2==1) //odd
    {
        ...
        LowerTriangularSolverRecursiveReal_3(matrixL2, matrixB2, matrixX2, n+1, n+1);
    ...
    else
    {
        ...
        double *L11 = (double *)malloc(nn2 * sizeof(double));
        double *L12 = (double *)malloc(nn2 * sizeof(double));
        double *L21 = (double *)malloc(nn2 * sizeof(double));
        double *L22 = (double *)malloc(nn2 * sizeof(double));
        double *B11 = (double *)malloc(nn2 * sizeof(double));
        double *B12 = (double *)malloc(nn2 * sizeof(double));
        double *B21 = (double *)malloc(nn2 * sizeof(double));
        double *B22 = (double *)malloc(nn2 * sizeof(double));
        ...
        // Recurse L11 X11 = B11
        LowerTriangularSolverRecursiveReal_3(L11, B11, X11, nn, nn);

        // Recurse L11 X12 = B12
        LowerTriangularSolverRecursiveReal_3(L11, B12, X12, nn, nn);
        ...
        for (int i = 0; i < nn; ++i)
        {
            for (int j = 0; j < nn; ++j)
            {
                for (int k = 0; k < nn; ++k)
                {
                    B21[i + (j * nn)] -= (L21[i + (k * nn)]) * (X11[k + (j)*nn]);
                    B22[i + (j * nn)] -= (L21[i + (k * nn)]) * (X12[k + (j)*nn]);
                    // Assumes even matrix (if not, you cannot mix these 2 and need j<pp2 )
                }
            }
        }
        ...
        // PHASE 3: RECURSE on REST of calculations with TRIANGULAR A22
        // Recurse L22 X21 = B21'
        LowerTriangularSolverRecursiveReal_3(L22, B21, X21, nn, nn);
        // Recurse L22 X22 = B22'
        LowerTriangularSolverRecursiveReal_3(L22, B22, X22, nn, nn);
...
int main(int argc, char *argv[])
{
    ...
    // Solve (1) LY=B
    start = std::chrono::high_resolution_clock::now();
    LowerTriangularSolverRecursiveReal_3(matrixL, matrixB, matrixY, n, n);
    // Solve (2) UX=Y
    UpperTriangularSolverRecursiveReal_3(matrixU, matrixY, matrixX, n,n);
    stop = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start);
